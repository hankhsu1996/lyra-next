...
module real_conversion;
  ...
  // @bits16 error[lyra.type.conversion_arg_type]: $bitstoreal requires 64-bit argument, got 16-bit

  // Wrong width for $bitstoshortreal (expects 32-bit, got 64-bit)
  assign sr = $bitstoshortreal(bits64);
  // @bits64 error[lyra.type.conversion_arg_type]: $bitstoshortreal requires 32-bit argument, got 64-bit

  // Category mismatch: $bitstoreal with real arg
  assign r = $bitstoreal(r);
  // @2:r error[lyra.type.conversion_arg_type]: $bitstoreal requires integral argument

  // Category mismatch: $realtobits with integral arg
  assign bits64 = $realtobits(bits64);
  // @2:bits64 error[lyra.type.conversion_arg_type]: $realtobits requires real argument
endmodule
