...
module packed_record_non_integral_member;
  ...
  typedef union packed {
    real r;
//       ^ error[lyra.semantic[16]]: union member `r` is not an integral type (real) in packed union
    logic [63:0] bits;
  } packed_union_real_t;

  typedef union packed {
    string s;
//         ^ error[lyra.semantic[16]]: union member `s` is not an integral type (string) in packed union
    logic [7:0] bits;
  } packed_union_string_t;

  typedef struct packed {
    real r;
//       ^ error[lyra.semantic[16]]: struct member `r` is not an integral type (real) in packed struct
    logic [7:0] b;
...
//                    ^ error[lyra.semantic[16]]: union member `us` is not an integral type (unpacked struct) in packed union
    logic [63:0] bits;
  } packed_union_unpacked_struct_t;

  typedef struct packed {
    logic [7:0] arr [4];
//              ^ error[lyra.semantic[16]]: struct member `arr` is not an integral type (unpacked array) in packed struct
  } packed_struct_array_t;

  typedef union soft packed {
    real r;
//       ^ error[lyra.semantic[16]]: union member `r` is not an integral type (real) in soft packed union
    logic [63:0] bits;
  } soft_packed_real_t;
