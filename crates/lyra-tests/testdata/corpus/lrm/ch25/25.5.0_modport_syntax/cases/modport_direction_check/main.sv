...
module write_to_input(my_bus.master bus);
  ...
  // @gnt error[lyra.type.modport_direction]: modport member declared 'input' cannot be used in write context
endmodule
...
module read_from_output(my_bus.slave bus);
  logic local_gnt;
  always_comb local_gnt = bus.gnt;
  // @gnt error[lyra.type.modport_direction]: modport member declared 'output' cannot be used in read context
endmodule

// Compound assignment on input member
module compound_on_input(my_bus.slave bus);
  always_comb bus.data += 8'h01;
  // @data error[lyra.type.modport_direction]: modport member declared 'input' cannot be used in write context
endmodule
